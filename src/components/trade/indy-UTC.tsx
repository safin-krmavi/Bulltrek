        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            try {
              ...
              if (!baseUrl) {
                alert("API base URL is not set. Please check your environment variables.");
                setLoading(false);
                return;
              }
              if (!accessToken || typeof accessToken !== "string" || accessToken.trim() === "") {
                alert("You are not logged in or token is missing. Please log in again.");
                setLoading(false);
                return;
              }
              accessToken = accessToken.trim();
              console.log("[IndyUTC] API URL:", baseUrl + "/strategies");
              console.log("[IndyUTC] Access token:", accessToken);
              const body = {
                name,
              ...
              if (!res.ok) {
                ...
                throw new Error(err.message || "Failed to create Indy UTC strategy.");
              }
              alert("Indy UTC strategy created successfully.");
            } catch (err: any) {
